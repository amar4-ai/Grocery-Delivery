...
const Checkout = () => {
  ...
  const {items, cartTotal, clearCart} = userCart()
  const {user} = useAuth()
  ...
  const [loading, setLoading] = useState(false)
   ...
   const handlePlaceOrder = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || error.message || "Failed to placed order")
    }finally {
      setLoading(false)
      scrollTo(0, 0)
    }
    ...
    return (
      <div className="min-h-screen bg-app-cream flex-center">
        <div className="text-center">
          <h2 className="text-xl font-semibold text-app-green mb-2">Your cart it
            empty
          </h2>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-2xl p-5 h-fit sticky top-24">
            <h3 className="text-sm font-semibold text-app-green mb-4">Order Summary
