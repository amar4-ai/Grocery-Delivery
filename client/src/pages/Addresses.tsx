...
const Addresses = () => {
  ...
  const getLocation = (retries = 3): Promise<{ lat: number, lng: number }> => {
    return new Promise((resolve, reject) => {
      if (!navigator.geolocation) {
        reject(new Error("Grolocation is not supported by your browser:"))
        return;
      }
        ...
        navigator.geolocation.getCurrentPosition(
          ...
          (error: any) => {
            if (retries > 0) {
              ...
              reject(new Error(error.message || "Failed to get geoLocation after retries"))
            }
          },
          {
  ...
  const handleSubmit = async (e: React.SubmitEvent) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || error.message || "Failed to submit address")
    }
  }
    ...
    <div className="min-h-screen bg-app-cream">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={() => { resetForm(); setShowFrom(true) }} className="px-4 py-2 bg-app-green text-white text-sm font-semibold
        ...
        {/* Form Modal */}
        {showFrom && <AddressForm resetForm={resetForm} handleSubmit={handleSubmit} form={form} setForm={setForm} editingId={editingId} />}
